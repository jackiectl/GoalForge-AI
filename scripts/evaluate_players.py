...
def main():
    ...
    for kind in ("scoring", "assist"):
        ...
        for label in data:
            ...
            gc = train_goal[col].value_counts()          # goals/assists per player (leakage-free)
            nn = train_app.groupby("player").minutes.sum() / 90.0
            d = data[label]
            lu = lineups[lineups.match_id.isin(set(d.matches.match_id))]

            def prate(p):
                return (gc.get(p, 0) + KAPPA * prior.get(posmap.get(p, "MF"), 0.08)) / (nn.get(p, 0.0) + KAPPA)
            ...
            for mid in d.matches.match_id:
                ...
                for team in row:
                    ...
                    rate = {p: pr.rate(p, kind) for p in players}
                    posrate = {p: prate(p) for p in players}
                    posscore = dict(zip(xi.player, xi.pos.map(lambda z: prior.get(z, 0.08))))
                    rankers = {"uniform": players, "position": _order(posscore),
                               "model": _order(rate), "model_pos": _order(posrate)}
                    for m in METHODS:
